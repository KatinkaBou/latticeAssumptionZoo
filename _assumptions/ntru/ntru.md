...
Similar to [R-LWE](/lwe/#ring-lwe_mqchimathcalr), several modern papers rather choose $f(X) = X^d + 1$ with $d$ a power of two and prime modulus $q \in 1 + 2d\ZZ$ to enable the [NTT](https://en.wikipedia.org/wiki/Discrete_Fourier_transform_over_a_ring#Number-theoretic_transform){:target="_blank"} fast matrix multiplication.
...
Nevertheless, the reduction sketched out in Section 4.4.4 of {% cite FTTCS:Peikert16 %} shows that [R-LWE](/lwe/#ring-lwe_mqchimathcalr) is at least as hard as NTRU, providing an _upper bound_ to the hardness of NTRU.
...
- [Ring-LWE](/lwe/#ring-lwe_mqchimathcalr) provides similar capabilities and operates on more general lattices.
