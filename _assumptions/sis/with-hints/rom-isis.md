...
### Randomised One-More-ISIS$$_{n,m,q,\beta,s}$$ {#randomised-one-more-isis}
_Let matrix $$\mat{A}, \mat{B} \in \ZZ_q^{n \times m}$$, and $$T \subset \ZZ_q^n$$ be chosen uniformly at random. Given the challenge matrices $$\mat{A}$$ and $$\mat{B}$$, and the set of target vectors $$T$$, an adversary can query a preimage oracle $$O_\text{pre}$$ adaptively, which on input $$\hat{\vec{t}} \in \ZZ_q^n$$ outputs a tuple $$(\hat{\vec{s}}, \hat{\vec{u}})$$ containing a preimage $$\hat{\vec{s}} \sample \mat{A}_s^{-1}(\hat{\vec{s}} - \mat{B} \cdot \hat{\vec{u}})$$ and a uniformly chosen vector $$\hat{\vec{u}} \sample \bin^m$$. Let $$k \in \NN_0$$ denote the number of times $$O_\text{pre}$$ was queried. Then, an adversary is asked to output a set $$\set{(\vec{s}_i, \vec{u}_i)}_{i \in [k+1]}$$ of $$k+1$$ short preimages of target vectors in $$T$$ satisfying_
